...
int main()
{
	...
	while (t--)
	{
		...
		vector<vector<int>> freq(N + 1, vector<int>(10, 0));
		for (int i{}; i < N; i++)
		{
			freq[i][str[i] - '0']++;
			freq[i + 1] = freq[i];
		...
		auto getFreq = [&](int l, int r, int j)
		{
			int res{};
			res += freq[r][j];
			res -= (l > 0 ? freq[l - 1][j] : 0);
			return res;
		};
		vector<Hash> vc;
		while (Q--)
		{
			...
			cin >> l >> r;
			--l, --r;
			Hash pref = strHash.get(0, l - 1);
			Hash suff = strHash.get(r + 1, N - 1);
			Hash sub;
			for (int j{}; j < 10; j++)
			{
				int f = getFreq(l, r, j);
				if (f > 0)
					sub = (sub + Hash({mult64(sumB1[f - 1], (j + '0')), mult64(sumB2[f - 1], (j + '0'))}, f));
			}
			vc.push_back(pref + sub + suff);
		}
