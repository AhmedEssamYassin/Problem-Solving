...
int main()
{
	...
	while (t--)
	{
		string str, b, e;
		cin >> str >> b >> e;
		int n = str.length(), nb = b.length(), ne = e.length();
		if (n < nb || n < ne)
			return cout << 0, 0;
		...
		HashRange hStr(str);
		vector<int> ind;
		for (int j = 0; j + ne - 1 < n; j++)
		{
			if (hStr.get(j, j + ne - 1) == he)
				ind.push_back(j + ne - 1);
		}

		vector<Hash> vc;
		int minLen = max(nb, ne);
		for (int i = 0; i + nb - 1 < n; i++)
		{
			if (hStr.get(i, i + nb - 1) == hb)
			{
				for (const int &j : ind)
					j - i + 1 >= minLen && (vc.push_back(hStr.get(i, j)), true);
			}
		}
