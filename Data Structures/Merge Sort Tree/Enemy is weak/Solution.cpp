...
private:
	...
	void build(int left, int right, int node, const vector<int> &arr)
	{
...
public:
	MergeSortSegmentTree(const vector<int> &arr)
	{
		size = 1;
		int n = arr.size();
		while (size < n)
...
int main()
{
	...
	while (t--)
	{
		...
		for (int i{}; i < n; i++)
			cin >> vc[i];
		MergeSortSegmentTree mSegTree(vc);
		ll cnt{};
		for (int j = 1; j < n - 1; j++)
		{
			ll cntL = mSegTree.countGreater(0, j - 1, vc[j]);
			ll cntR = mSegTree.countLess(j + 1, n - 1, vc[j]);
			cnt += cntL * cntR;
		}
		cout << cnt;
	}
