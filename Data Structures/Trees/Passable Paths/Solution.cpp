...
int main()
{
	...
	while (t--)
	{
		...
		while (Q--)
		{
			...
			sort(node.begin(), node.end(), [&](ll u, ll v)
				 { return treeAnc.getDepth(u) > treeAnc.getDepth(v); });
			// Find the deepest node
			ll a = node[0];
			for (int i = 1; i < k; i++)
				if (treeAnc.getDepth(node[i]) > treeAnc.getDepth(a))
					a = node[i];

			// Find farthest from a
			ll b = node[0];
			for (int i = 1; i < k; i++)
				if (treeAnc.getDistance(a, node[i]) > treeAnc.getDistance(a, b))
					b = node[i];

			for (int i = 0; i < k; i++)
				check &= treeAnc.onPath(node[i], a, b);
