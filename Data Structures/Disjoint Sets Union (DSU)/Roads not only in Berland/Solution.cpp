...
int main()
{
	...
	while (t--)
	{
		cin >> N;
		DSU disjointUnion(N);
		vector<pair<ll, ll>> redundant;
		ll a, b;
		for (int i{}; i < N - 1; i++)
		{
			cin >> a >> b;
			if (not disjointUnion.isSameComponent(a, b))
				disjointUnion.Union(a, b);
			else
				redundant.push_back({a, b});
		}
		set<int> st = disjointUnion.findConnectedComponents();
		ll current = *st.begin();
		st.erase(current);
		cout << redundant.size() << endl;
		for (int i{}; i < redundant.size(); i++)
		{
			cout << redundant[i].first << " " << redundant[i].second << " " << current << " " << *st.begin() << endl;
			st.erase(*st.begin());
		}
	}
	return 0;
