...
struct DSU
{
	vector<ll> parent, size, maximum, minimum; // Representative
	int N;
	...
	DSU(int _N)
	{
		...
		iota(maximum.begin(), maximum.end(), 0);
		iota(minimum.begin(), minimum.end(), 0);
	}
	ll Find(ll node)
	{
		if (parent[node] == node)
	...
	void Union(ll u, ll v)
	{
		...
		parent[rep1] = rep2;
		size[rep2] += size[rep1];
		maximum[rep2] = max(maximum[rep1], maximum[rep2]);
		minimum[rep2] = min(minimum[rep1], minimum[rep2]);
	}
	...
	tuple<ll, ll, ll> query(ll node)
	{
		ll rep = Find(node);
		return {minimum[rep], maximum[rep], size[rep]};
	}
};
...
int main()
{
	...
	while (t--)
	{
		...
		while (M--)
		{
			...
			{
				cin >> v;
				tuple<ll, ll, ll> ans = disjointUnion.query(v);
