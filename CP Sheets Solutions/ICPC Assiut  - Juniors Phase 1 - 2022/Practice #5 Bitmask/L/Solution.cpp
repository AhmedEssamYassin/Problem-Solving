...
int main()
{
	...
	while (t--)
	{
		...
		In each time we add 1, we flip ALL bits after the rightmost cleared bit of previous number, including the rightmost cleared bit
		...
		*/
		...
		ll val = (L ^ R);
		int shift = 63 - __builtin_clzll(val);
		cout << (1LL << (shift + 1)) - 1;
	}
	return 0;
}