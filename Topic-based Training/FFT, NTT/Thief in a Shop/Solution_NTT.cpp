...
*/
const auto NTT_MOD = 469762049;
const decltype(NTT_MOD) PRIMITIVE_ROOT = 3; // Primitive root for 469762049
...
vector<T> PolyModPow(vector<T> P, ll power)
{
	...
	{
		if (power & 1)
			res = convolute(res, P), adjust(res);
		P = convolute(P, P), adjust(P);
		power >>= 1;
	}
	return res;
}
...
int main()
{
	...
	while (t--)
	{
		...
		for (int i{}; i < N; i++)
			cin >> costs[i];
		int sz = *max_element(costs.begin(), costs.end());
		vector<int> P(sz + 1);
		for (int i{}; i < N; i++)
			P[costs[i]] = 1;
		vector<int> ans = PolyModPow(P, k);
