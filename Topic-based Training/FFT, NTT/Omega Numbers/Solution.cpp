...
int main()
{
	...
	while (t--)
	{
		...
		vector<vector<Mint>> freq(8, vector<Mint>(n + 1, 0));
		for (int i = 0; i < n; i++)
			freq[omega[a[i]]][a[i]]++;
		Mint ans = 0;
		for (int u = 0; u < 8; u++)
		{
			for (int v = u; v < 8; v++)
			{
				...
				if (uEmpty || vEmpty)
					continue;
				vector<Mint> res = convoluteGCD(freq[u], freq[v]);
				for (int g = 1; g <= n; g++)
				{
					Mint cnt = res[g];
					if (u == v)
					...
					if (!IsZero(cnt))
					{
						Mint base = u + v - omega[g];
						ans += (cnt * power(base, k));
					}
				}
			}
		}
